...
const createFileController=async(req: Request, res: Response)=> {
    try {
        const {file_name, file_size, folder_id, category_id, description } = req.body;
        const file=req.file;
...
const getAllFilesController = async (req: Request, res: Response): Promise<void> => {
  try {
...
const updateFileController = async (req: Request, res: Response): Promise<void> => {
  try {
    if (req.file){
     
          const firebaseUrl = await uploadToFirebase(req.file.path, req.body.file_name);
...
export {createFileController, getAllFilesController, getFileController, updateFileController, deleteFileController}
