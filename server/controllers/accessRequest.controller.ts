...
// Controller function to create an access request
const createAccessRequestController = async (req: Request, res: Response) => {
  try {
    ...
    if (!file_id || !user_id || !reason) {
      return res.status(400).json({ message: 'user_id,file_id and reason are required' });
    }
...
const updateAccessRequestController = async (req: Request, res: Response) => {
  try {
    ...
    if (!file_id || !status || !reason) {
      return res.status(400).json({ message: 'File ID, status, and reason are required' });
    }
...
const deleteAccessRequestByIdController = async (req: Request, res: Response) => {
  try {
    const requestId: number = parseInt(req.params.id, 10);
